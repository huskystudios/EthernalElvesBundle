...
let query = new Moralis.Query(Elves);
  ...
  let owner = request.params.address.toLowerCase()
 
  query.equalTo("owner_of", owner);
...
Moralis.Cloud.define("signForEthReturn", async(request) => {
  ...
  const objectId =  request.params.objectIds 
  const asset = request.params.asset
  let response
  
  if(asset === "ren"){

      let q = new Moralis.Query("ElvesRenTransferIn");  
      q.equalTo("objectId", objectId);
      const s = await q.first();

      const renAmount =  s.get("renAmount") 
      const walletAddress = s.get("from") 
      const timestamp = s.get("timestamp") 
      ...
      s.set("signedTransaction", signedTransaction)
      s.save(); 
    
  response = { signedTx: signedTransaction, checksumawallet: checkSumAddress, solidhash: solidhash,  walletAddress: walletAddress}
      ...
      const tokenId =  s.get("tokenId") 
      const sentinel = s.get("sentinel") 
      const walletAddress = s.get("from") 

      const checkSumAddress = ethWeb3.utils.toChecksumAddress(walletAddress);
      const authHash = ethWeb3.utils.soliditySha3(objectId)
      const solidhash = ethWeb3.utils.soliditySha3(tokenId, checkSumAddress, sentinel, authHash)
      const signedTransaction = ethWeb3.eth.accounts.sign(solidhash,validatorKey);	

      s.set("signedTransaction", signedTransaction)
      s.set("authCode", authHash)
      s.save(); 
 ...
 if (confirmed) {
   ...
   query.equalTo("renAmount", renAmount);
    ...
    if(!res){   
       ...
       logger.info(checkSumAddress)
       logger.info(solidhash)
       logger.info(signedTransaction.signature)

    transferObject.set("signature", signedTransaction.signature)
    ...
    logger.info("Made it here!")
    	...
    	logger.info("Dit not find it here!")
    }
 }
...
Moralis.Cloud.define("adminTransactor", async(request) => {
